...
wiki = WikiCorpus("ruwiki-full-articles.bz2")
...
lastTime = datetime.now()
model.save("wiki-full-rus-articles-03032020.model")
print("Model Saved !!!!!!!!!!!")
...
for model in models:
    print(str(model))
    pprint(model.docvecs.most_similar(positive=[model.infer_vector("Красная армия".split())], topn=10))

for model in models:
    print(str(model))
    pprint(model.docvecs.most_similar(positive=[model.infer_vector("Пушкин Александр Сергеевич".split())], topn=10))
